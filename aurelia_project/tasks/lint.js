...
const main = gulp.series(templateLint, failOnErrors);

function templateLint() {
    function mustIgnoreError(error, file) {
        if (
            file.endsWith(path.join("app", "game", "play", "widgets", "trump", "trump.html")) &&
    ...
    return gulp.src("./app/**/*.html").pipe(
        aureliaTemplateLint(null, (error, file) => {
            if (!mustIgnoreError(error, file)) {
                foundLintErrors = true;
                console.log(
                    `[Template lint] ${error.message} [file: ${file} ln: ${error.line} col: ${error.column}]`,
                );
