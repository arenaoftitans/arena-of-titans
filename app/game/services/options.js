...
export class Options {
    constructor(storage, observerLocator) {
        ...
        for (let key of Object.keys(this)) {
            observerLocator.getObserver(this, key).subscribe(() => this._storage.saveOptions(this));
        }
    }
    ...
    mustViewInGameHelp(name) {
        switch (name.toLowerCase()) {
            case "assassination":
                return (
                    this.proposeInGameHelp && !this.inGameHelpSeen.includes(ASSASSIN_IN_GAME_HELP)
                );
            default:
